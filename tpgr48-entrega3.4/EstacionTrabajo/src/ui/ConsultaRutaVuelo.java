...
public class ConsultaRutaVuelo extends JInternalFrame {
	...
	IUsuariosController controlUsr;
	IRutaVueloController controlRV;

	DataAerolinea[] aerolineas;
	DataAerolinea aerolinea;
	ArrayList<DataRutaVuelo> rutasVuelo;
	DataRutaVuelo rutaVuelo;
	private JButton btnAerolinea;
	private JButton btnRutasVuelo;
	...
	public ConsultaRutaVuelo() {
		...
		setTitle("Consultar ruta de vuelo");
		setBounds(100, 100, 839, 555);
		...
		gbc_comboBoxAerolinea.insets = new Insets(0, 0, 5, 5);
		...
		lblRutasVuelo = new JLabel("Rutas de vuelo:");
		...
		gbc_lblRutasVuelo.gridx = 0;
		gbc_lblRutasVuelo.gridy = 1;
		getContentPane().add(lblRutasVuelo, gbc_lblRutasVuelo);

		btnAerolinea = new JButton("Buscar");
		GridBagConstraints gbc_btnAerolinea = new GridBagConstraints();
		gbc_btnAerolinea.anchor = GridBagConstraints.WEST;
		gbc_btnAerolinea.insets = new Insets(0, 0, 5, 0);
		gbc_btnAerolinea.gridx = 2;
		gbc_btnAerolinea.gridy = 0;
		getContentPane().add(btnAerolinea, gbc_btnAerolinea);
		btnAerolinea.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent event) {
				if (comboBoxAerolinea.getSelectedIndex() != 0 && comboBoxAerolinea.getSelectedIndex() != -1) {
					cmdBuscarAerolinea(event);
		...
		gbc_comboBoxRutasVuelo.gridx = 1;
		gbc_comboBoxRutasVuelo.gridy = 1;
...
//		textField_10.setColumns(10);
		...
		btnRutasVuelo = new JButton("Buscar");
		btnRutasVuelo.setVisible(false);
		GridBagConstraints gbc_btnRutasVuelo = new GridBagConstraints();
		gbc_btnRutasVuelo.anchor = GridBagConstraints.WEST;
		gbc_btnRutasVuelo.insets = new Insets(0, 0, 5, 0);
		gbc_btnRutasVuelo.gridx = 2;
		gbc_btnRutasVuelo.gridy = 1;
		getContentPane().add(btnRutasVuelo, gbc_btnRutasVuelo);
		btnRutasVuelo.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent event) {
				//cmdBuscarRutasVuelo(event);
				if (comboBoxRutasVuelo.getSelectedIndex() != 0 && comboBoxRutasVuelo.getSelectedIndex() != -1) {
					...
					rv = rv.split(" - ", 2)[0];
					try {
						rutaVuelo = controlRV.verInfoRutaVuelo(rv);
	...
	protected void cmdBuscarAerolinea(ActionEvent event) {
		if (comboBoxAerolinea.getSelectedItem() != null) {
			lblRutasVuelo.setVisible(true);
			comboBoxRutasVuelo.setVisible(true);
			btnRutasVuelo.setVisible(true);

		} else {
			JOptionPane.showMessageDialog(this, "No hay ninguna aeroliena seleccionado", "Consultar ruta de vuelo",
					JOptionPane.ERROR_MESSAGE);
		}
	}

	protected void cmdBuscarRutasVuelo(ActionEvent event) {
		if (comboBoxRutasVuelo.getSelectedItem() != null) {
			...
			JOptionPane.showMessageDialog(this, "No hay ninguna ruta de vuelo seleccionada.", "Consultar ruta de vuelo",
					JOptionPane.ERROR_MESSAGE);
		}
	}
